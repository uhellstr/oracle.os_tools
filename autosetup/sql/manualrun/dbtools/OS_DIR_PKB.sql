...
as
 ...
 begin
  ...
  procedure add_ext_tab_for_new_dir
    --*===========================================================================
    --* NAME:        add_ext_tab_for_new_dir
    --*
    --* DESCRIPTION: This procedure will add a external table for any new
    --*              directory that either is granted to or create as DBTOOLS
    --*
    --* CREATED:     2019-01-22
  ...
  is
    ...
    cursor cur_get_dirs_without_ext_tab is
    select a.t_directory_name as directory, 
           a.t_path as path
    from table(os_tools.get_directory_names) a
    where a.t_directory_name not in (
    select replace(b.table_name,'_EXT_TAB','') 
    from dba_tables b
    where b.owner = 'DBTOOLS'
      and b.table_name like '%EXT_TAB'
      and a.t_directory_name = replace(b.table_name,'_EXT_TAB','')
    );
    
  begin  
  
    for rec in cur_get_dirs_without_ext_tab loop
      setup_dir(rec.directory,'DBTOOLS');
    end loop;
    
  end add_ext_tab_for_new_dir;
